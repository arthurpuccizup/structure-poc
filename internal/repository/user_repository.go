...
func (r userRepository) FindAll() ([]domain.User, error) {
	...
	if res := r.db.Find(&users); res.Error != nil {
		return nil, logging.NewError("Find all users failed", res.Error, nil, "repository.FindAll.Find")
	}
...
func (r userRepository) FindAllCustom() ([]domain.User, error) {
	...
	if res := r.db.Raw(r.queries["find-all-custom"]).Scan(&users); res.Error != nil {
		return nil, logging.NewError("Find all users failed", res.Error, nil, "repository.FindAllCustom.Find")
	}
...
func (r userRepository) Create(user domain.User) (domain.User, error) {
	...
	if res := r.db.Save(&userToSave); res.Error != nil {
		return domain.User{}, logging.NewError("Save user failed", res.Error, nil, "repository.Create.Save")
	}
...
func (r userRepository) GetByID(id uuid.UUID) (domain.User, error) {
	...
	if res := r.db.Model(models.User{}).Where("id = ?", id).First(&user); res.Error != nil {
		return domain.User{}, logging.NewError("Find user failed", res.Error, nil, "repository.GetById.First")
	}
...
func (r userRepository) Update(id uuid.UUID, user domain.User) (domain.User, error) {
	...
	if res := r.db.Model(models.User{}).Where("id = ?", id).Updates(&userToUpdate); res.Error != nil {
		return domain.User{}, logging.NewError("Update user failed", res.Error, nil, "repository.Update.Updates")
	}
...
func (r userRepository) Delete(id uuid.UUID) error {
	if res := r.db.Delete(models.User{}, id); res.Error != nil {
		return logging.NewError("Delete user failed", res.Error, nil, "repository.Delete.Delete")
	}
