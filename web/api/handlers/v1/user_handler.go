...
func CreateUser(saveUser userInteractor.SaveUser) echo.HandlerFunc {
	return func(c echo.Context) error {
		logger := c.Request().Context().Value(observ.LoggerFlag).(*zap.SugaredLogger)
		var user representation.UserRequest
		bindErr := c.Bind(&user)
		if bindErr != nil {
			logger.Error(bindErr)
			return c.JSON(http.StatusInternalServerError, observ.New("Cant parse body", bindErr, nil))
		}

		validationErr := c.Validate(user)
		if validationErr != nil {
			logger.Error(validationErr)
			return c.JSON(http.StatusInternalServerError, observ.New("Invalid Fields", validationErr, nil))
		}
...
func FindUserById(findUserById userInteractor.FindUserById) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if parseErr != nil {
			...
			return c.JSON(http.StatusInternalServerError, observ.New("Parse id failed", parseErr, nil))
		}
